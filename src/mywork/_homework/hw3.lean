...
ANSWER HERE:
Plato is a person, and given that being mortal is a characteristic of 
all humans, we can deduce that Plato is mortal since all people are mortal.
-/
...
variable Triangle : ∀ (p1 p2 p3 : Person), Likes p1 p2 → Likes p2 p3 → Jealous p1 p3
...
Answer:
If we are given the propositions P and Q, and P and Q is true, 
we can use and elimination left to deduce that P is true and we can use
and elimination right to deduce that Q is true. Then we can use the
or elimination introduction rule to prove that Q ∨ P is true.
-/
...
def answer : Prop := 
  ∀ (everyone: Person), ∃ (knows_someone: Person), ∃ (someone_knows_someone: Person), ∀ (everyone_else: Person), 
  (Knows everyone knows_someone) ∧ 
  (Knows knows_someone someone_knows_someone) ∧ 
  (Knows someone_knows_someone everyone_else) → 
  (Knows everyone everyone_else)