...
-- o	The set of all s of type string such that s.length equals 5
...
example : strings5 "Lean!" :=  --- predicate application notation
begin                     -- strings5 "Lean!"
...
example : 7 ∈ my_universal_set nat := -- predicate applied to 7, what does that reduce to? it reduces to true 
begin 
trivial, --a tactic to prove true, prove true.intro etc
end
...
def my_set_complement
  ...
  (S : set α) 
  (a : α):
  a ∉ S ↔ a ∈ Sᶜ -- a being in the complement of S means a is not in S
  :=
begin
split,
assume h,
