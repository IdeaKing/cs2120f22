...
/- 
for all propositions x and y, if x and y are true, then x is true
-/
...
/- 
for all prop x and y if x is true then x and y is true
if you give me a proof of x i can build a proof of x and y
-/
def or_intro_right : Prop   := Y → X ∨ Y
def or_elim : Prop          := (X ∨ Y) → (X → Z) → (Y → Z) → Z
/- 
for all propositions x, y, and z, if x or y is true, and if x is true 
then z is true, and if y is true then z is true, then z is true

if i have a proof of x or y then if i have a proof of x implies z \
then if i have a proof of y implies z, then z

if i stepped in dog litter or i stepped in mud, then my shoe is brown 
and if i stepped in dog litter then my shoe is brown and if i stepped
in mud then my shoe is brown, then my shoe is brown
-/
...
assume x, y are arbitrary proposition. lets assume we have a proof of
x or y, what remains to preove is x or x.

apply the rule of or_elim to X ∨ Y 
case 1: x is true or we have a proof of it, or_intro on the right → Y ∨ X is True
case 2: y is true or we have a proof of it, or_intro on the left → Y V X is True

Y → X is true in either case

if you have a proof of x or y DO CASE ANALYSIS ON THE PROPOSITION

if its raining cats or its raining dogs then its raining dogs or its raining cats.
-/
...
/- 
Every puppy is happy, for every puppy, p p is happy
∀ (p : Puppy), Happy p

assume p is any puppy
show p is happy
then every p is happy
∀ p, Happy p

supposed I have an arbitrary puppy, and this arbitrary 
puppy is happy, then all puppies are happy

arbitrary but specific object

after proving an universtal generalizatinon then I can pick
a specific (universal specialization) object of the specification

i can take a generalization and i can take a specific object and 
derive a truth about that specific object 
-/
...
/- 
if i have a proof of x implies y and i have a proof that y implies 
x then they imply each other (equivalent)
